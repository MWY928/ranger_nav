...
def _select_checkpoint_state_dict(ckpt_obj):
    if isinstance(ckpt_obj, dict) and 0 in ckpt_obj and "state_dict" in ckpt_obj[0]:
...
def _resolve_sample(path: str) -> str:
    if os.path.isdir(path):
        matches = sorted(glob.glob(os.path.join(path, "bridge_policy_replay_*.json")))
        if len(matches) == 0:
            raise RuntimeError("No bridge_policy_replay_*.json found in {}".format(path))
        return matches[-1]
    return path
...
def _build_policy(args, depth_shape, depth_key: str, goal_key: str):
    ...
    state_dict = _extract_actor_critic_state_dict(_select_checkpoint_state_dict(ckpt))
...
def _fmt_probs(probs: Optional[np.ndarray]) -> str:
    ...
    return " ".join(
        "{}:{:.6f}".format(labels[i] if i < len(labels) else "a{}".format(i), float(p))
        for i, p in enumerate(probs.tolist())
    )


def main():
    parser = argparse.ArgumentParser(description="Replay a saved Falcon bridge policy input")
    ...
    parser.add_argument("--zero_state", action="store_true", help="Ignore saved RNN input state")
    ...
    policy, missing, unexpected = _build_policy(args, depth.shape, depth_key, goal_key)
    ...
    if args.zero_state:
        ...
        prev_action = torch.as_tensor(data["prev_action_in"], dtype=torch.long, device=args.device)
        mask = torch.as_tensor(data["not_done_mask_in"], dtype=torch.bool, device=args.device)
    ...
    saved_action = int(data["action"][0]) if "action" in data else meta.get("action_id")
    saved_probs = data["probs"] if "probs" in data else np.array([])
    saved_value = float(data["value"][0]) if "value" in data else meta.get("critic_value")

    print("sample: {}".format(sample_path))
    print("checkpoint: {}".format(args.checkpoint))
    print("load_state_dict missing={} unexpected={}".format(len(missing), len(unexpected)))
    print("depth shape={} min={:.6f} max={:.6f}".format(depth.shape, float(depth.min()), float(depth.max())))
    print("goal [r, theta]=[{:.6f}, {:.6f}]".format(float(goal[0]), float(goal[1])))
    print("saved action={} value={:.6f} probs=[{}]".format(saved_action, float(saved_value), _fmt_probs(saved_probs)))
    print("replay mode action={} value={:.6f} probs=[{}]".format(replay_action, replay_value, _fmt_probs(probs)))
    if saved_probs is not None and saved_probs.size > 0 and probs is not None:
        print("max prob diff={:.9f}".format(float(np.max(np.abs(saved_probs - probs)))))
    print("action match={}".format(saved_action == replay_action))
